...
### 2.2 Program Output (stdout/stderr) ⚠️ **MIGRATION IN PROGRESS**

| Output Type | Language | Status |
|:---|:---|:---|
| **stdout / stderr** | **English only** | Enforced for new code |
| **Logging** | **English only** | Enforced |
| **HDF5 metadata** | **English only** | Enforced |
| **Exception messages** | **English only** | Enforced |

**Migration Note**: Existing Chinese stdout output is being phased out gradually. All new code must comply.
...
- [ ] HDF5 file attributes/descriptions are in English?
